...
public enum ConsoleUiMessage {
    ...
    private final String userInputKey;// 각 상수에 연결된 사용자 입력 키를 저장하는 필드

    ConsoleUiMessage(String userInputKey) {// 생성자: enum 상수가 생성될 때 호출되어 키 값을 초기화
        this.userInputKey = userInputKey;// this.userInputKey 필드에 매핑된 입력 키 저장
    }

    public static ConsoleUiMessage fromUserInput(String input) {
        for (ConsoleUiMessage message : ConsoleUiMessage.values()) {// 모든 열거형 상수 순회
            if (message.userInputKey.equals(input)) {// 상수에 저장된 키와 사용자의 입력이 일치하는지 비교
